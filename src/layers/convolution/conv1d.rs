    ...
    fn feed_forward<F: FnMut([usize; 2], T)>(&self, input: &Tensor<T>, mut f: F) -> Tensor<T> {
        ...
        self.iter_through_output(|[o0, o1]| {
            ...
            self.iter_through_kernel(|[k0, k1]| {
                ...
                if let Some(i0) = i0 {
                    let groups = self.input_shape[1] / self.kernel.get_shape()[1];
                    for g in (0..groups).map(|g| g * self.kernel.get_shape()[1]) {
...
impl<
    ...
    KC: Constraint<T>,
    BC: Constraint<T>,
> Layer for Conv1D<T, A, KR, BR, AR, KC, BC> {
