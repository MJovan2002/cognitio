        ...
        for k0 in 0..self.kernel.get_shape()[0] {
            for k1 in 0..self.kernel.get_shape()[1] {
                for k2 in 0..self.kernel.get_shape()[2] {
                    for k3 in 0..self.kernel.get_shape()[3] {
                        f([k0, k1, k2, k3])
                    }
                }
            }
        ...
        self.iter_through_output(|[o0, o1, o2, o3]| {
            ...
            self.iter_through_kernel(|[k0, k1, k2, k3]| {
                ...
                if let (Some(i0), Some(i1), Some(i2)) = (i0, i1, i2) {
                    let groups = self.input_shape[3] / self.kernel.get_shape()[3];
                    for g in (0..groups).map(|g| g * self.kernel.get_shape()[3]) {
...
impl<
    ...
    KC: Constraint<T>,
    BC: Constraint<T>,
> Layer for Conv3D<T, A, KR, BR, AR, KC, BC> {
