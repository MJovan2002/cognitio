...
pub struct Pooling1D<S, T: Number> {
    input_shape: Shape,
    output_shape: Shape,
    pool_size: [usize; 1],
...
impl<T: Number + From<i32>> Pooling1D<Average, T> {
    fn feed_forward<F: FnMut([usize; 2], T)>(&self, input: &Tensor<T>, mut f: F) -> Tensor<T> {
...
impl<T: Number + From<i32>> Layer for Pooling1D<Average, T> {
...
impl<T: Number + Ord> Pooling1D<Max, T> {
    fn feed_forward_<F: FnMut([usize; 2], [usize; 2])>(&self, input: &Tensor<T>, mut f: F) -> Tensor<T> {
        let mut output = Tensor::zero(self.output_shape.clone());
        self.iter_through_output(|[o0, o1]| {
            let mut o = T::zero();
            let mut mi = [0, 0];
...
impl<T: Number + Ord> Layer for Pooling1D<Max, T> {
    const INPUT_DIMENSION: usize = 1;
    type InputType = T;
    const REVERSE_INPUT_DIMENSION: usize = 1;
    type ReverseInputType = T;
